...
public class ObjectManager {
    ...
    private BufferedImage estatuaImg;
    ...
    private ArrayList<Statue> estatuas; 
    ...
    public void checkObjectHit(Rectangle2D.Float attackbox) {
        for (Statue estatua : estatuas) {
            if (estatua.isActive() && !estatua.isAcertijoResuelto()) {
                if (estatua.getHitbox().intersects(attackbox)) {
                    
                    String respuestaJugador = JOptionPane.showInputDialog(
                        null, 
                        estatua.getPregunta(), 
                        "La Estatua Misteriosa Habla...", 
                        JOptionPane.QUESTION_MESSAGE
                    );
                    
                    if (respuestaJugador != null && respuestaJugador.equalsIgnoreCase(estatua.getRespuestaCorrecta())) {
                        JOptionPane.showMessageDialog(null, "¡Correcto! Una poción aparece de la nada.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
                        estatua.setAcertijoResuelto(true);
                        
                        int dropX = (int) (estatua.getHitbox().x);
                        int dropY = (int) (estatua.getHitbox().y - estatua.getHitbox().height);
                        potions.add(new Potion(dropX, dropY, (Math.random() > 0.5) ? RED_POTION : BLUE_POTION));
                        
                        playing.increaseScore(500);
                    } else {
                        JOptionPane.showMessageDialog(null, "Respuesta incorrecta... Sientes un dolor punzante.", "Maldición", JOptionPane.ERROR_MESSAGE);
...
public void interactWithStatue(Rectangle2D.Float playerHitbox) {
        for (Statue estatua : estatuas) {
            if (estatua.isActive() && !estatua.isAcertijoResuelto()) {
                
                // Creamos un área de interacción
                Rectangle2D.Float areaDeInteraccion = new Rectangle2D.Float(
                    estatua.getHitbox().x - 20, 
                    estatua.getHitbox().y - 10, 
                    estatua.getHitbox().width + 40, 
                    estatua.getHitbox().height + 20
                );

                if (areaDeInteraccion.intersects(playerHitbox)) {
                    ...
                    if (respuestaJugador != null && respuestaJugador.equalsIgnoreCase(estatua.getRespuestaCorrecta())) {
                        JOptionPane.showMessageDialog(null, "¡Correcto! Una recompensa aparece de la nada.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
                        estatua.setAcertijoResuelto(true);
                        
                        int dropX = (int) (estatua.getHitbox().x);
                        int dropY = (int) (estatua.getHitbox().y - estatua.getHitbox().height);
                        
                        // --- LÓGICA DE RECOMPENSA ALEATORIA TOTAL ---
                        // Elegimos un número entre 0, 1 y 2
                        int tipoRecompensa = (int) (Math.random() * 3); 
                        
                        if (tipoRecompensa == 0) {
                            // 0 = Da una poción (50% Roja, 50% Azul)
                            int tipoPocion = (Math.random() > 0.5) ? RED_POTION : BLUE_POTION;
                            potions.add(new Potion(dropX, dropY, tipoPocion));
                            
                        } else if (tipoRecompensa == 1) {
                            // 1 = Da un arma aleatoria (del 0 al número total de armas)
                            int randomWeapon = (int) (Math.random() * Utilz.Constantes.WeaponConstants.NUM_WEAPONS);
                            weapons.add(new Weapon(dropX, dropY, randomWeapon));
                            
                        } else {
                            // 2 = Da una armadura aleatoria (del 0 al número total de armaduras)
                            int randomArmor = (int) (Math.random() * Utilz.Constantes.WeaponConstants.NUM_ARMORS);
                            armors.add(new Armor(dropX, dropY, randomArmor));
                        }
                        // --------------------------------------------
    ...
    public void loadObjects(Level newLevel) {
        ...
        estatuas = new ArrayList<>(newLevel.getStatues()); // NUEVO
        projectiles.clear();
    }
    ...
    private void loadImgs() {
        ...
        estatuaImg = LoadSave.GetSpriteAtlas(LoadSave.STATUE_SPRITE); // NUEVO
    }
    ...
    public void draw(Graphics g, int xLvlOffset) {
        ...
        drawEstatuas(g, xLvlOffset); // NUEVO
    }

    // --- NUEVO: MÉTODO PARA DIBUJAR ESTATUAS ---
    private void drawEstatuas(Graphics g, int xLvlOffset) {
        for (Statue s : estatuas) {
            if (s.isActive()) {
               g.drawImage(estatuaImg, 
    (int) (s.getHitbox().x - s.getxDrawOffset() - xLvlOffset), 
    (int) (s.getHitbox().y - s.getyDrawOffset()), 
    (int) (32 * Juego.SCALE), // <-- Aquí agregamos el (int)
    (int) (40 * Juego.SCALE), // <-- Aquí también
    null);
            }
        }
    }
    ...
    public void resetAllObjects() {
        ...
        // Reiniciar las estatuas para que vuelvan a tener el acertijo sin resolver
        for (Statue s : estatuas) s.setAcertijoResuelto(false);
    }
}