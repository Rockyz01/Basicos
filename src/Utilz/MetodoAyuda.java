...
import Objects.Weapon; // NUEVO IMPORT
...
public class MetodoAyuda {
    ...
    public static void SnapEntityToFloor(Rectangle2D.Float hitbox, int[][] lvlData) {
        if (lvlData == null) return;
        int colLeft  = (int) (hitbox.x / Juego.TILES_SIZE);
        int colRight = (int) ((hitbox.x + hitbox.width) / Juego.TILES_SIZE);
        int rowStart = (int) ((hitbox.y + hitbox.height) / Juego.TILES_SIZE);
        if (rowStart < 0) rowStart = 0;
        int bestFloorRow = -1;
        for (int row = rowStart; row < lvlData.length; row++) {
            for (int col = colLeft; col <= colRight; col++) {
                if (col < 0 || col >= lvlData[0].length) continue;
                if (IsTileSolid(col, row, lvlData)) {
                    bestFloorRow = row;
                    break;
                }
            }
            if (bestFloorRow != -1) break;
        }
        if (bestFloorRow != -1) {
            hitbox.y = bestFloorRow * Juego.TILES_SIZE - hitbox.height;
        }
    }
    ...
    // --- NUEVO: MÉTODO PARA LA ESTATUA ---
    public static ArrayList<Statue> GetStatues(BufferedImage img) {
        ArrayList<Statue> list = new ArrayList<>();
        for (int j = 0; j < img.getHeight(); j++) {
            for (int i = 0; i < img.getWidth(); i++) {
                Color color = new Color(img.getRGB(i, j));
                int valor = color.getBlue();
                if (valor == STATUE) {
                    list.add(new Statue(i * Juego.TILES_SIZE, j * Juego.TILES_SIZE, valor));
                }
            }
        }
        return list;
